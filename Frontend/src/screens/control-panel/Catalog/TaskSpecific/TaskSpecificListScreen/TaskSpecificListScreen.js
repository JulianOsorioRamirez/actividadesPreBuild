...
import ViewTaskInfoModal from './components/ViewTaskSpecificInfoModal'
import ActiveAndDesactiveTaskModal from './components/ActiveAndDesactiveTaskSpecificModal'
import SnackbarContent from 'components/Snackbar/SnackbarContent'
import UpdateTaskModal from './components/UpdateTaskSpecificModal'
import DeleteTaskModal from './components/DeleteTaskSpecificModal'
...
import styles from './styles/taskSpecificListScreenStyles'

const useStyles = makeStyles(styles)

const TaskSpecificListScreen = () => {
  ...
  const { loadingTaskSpecificList, taskSpecifics, successTaskSpecificList, errorTaskSpecificList } = useSelector((state) => state.taskSpecificList)

  useEffect(() => {
    if (successTaskSpecificList) {
      const list = taskSpecifics.map((item) => {
        return {
          ...item,
  ...
  }, [dispatch, successTaskSpecificList])
  ...
  const showUpdateTaskHandler = (id) => {
    const task = taskSpecifics.find((task) => task.id_tarea === id)
      ...
      <GridContainer>
        <GridItem xs={12} className={classes.rootItem}>
          <Card>
                ...
                {excel && (
                  ...
                  >
                    <ExcelFile.ExcelSheet data={data} name='Tareas'>
                      ...
                      <ExcelFile.ExcelColumn label='NOMBRE' value='fullname' />
            ...
            {errorTaskSpecificList && (
              <GridContainer>
                <GridItem xs={12}>
                  <SnackbarContent message={errorTaskSpecificList} color='danger' />
                </GridItem>
              </GridContainer>
            )}
          </Card>
...
export default TaskSpecificListScreen
